...
Configure how the SDK waits for DOM changes to stabilize before taking screenshots.

**Debounce Strategy:**

-   Waits for a period of inactivity after changes
-   **Debounce Delay:** Time to wait after last change (default: 300ms)
-   **Max Wait Time:** Maximum time to wait (default: 2000ms)

**Fixed Strategy:**

-   Waits a fixed amount of time after any change
-   **Wait Time:** Fixed delay in milliseconds (default: 500ms)
...
<Tip>
    Use debounce for dynamic applications with frequent updates. Use fixed for
...
interface MossConfig {
    ...
    // DOM Stability
    stability?: {
        strategy: "debounce" | "fixed";
        debounceDelay?: number; // For debounce strategy (default: 300ms)
        maxWaitTime?: number; // Max wait for debounce (default: 2000ms)
        waitTime?: number; // For fixed strategy (default: 500ms)
    };
}
```
...
2. **Stability Strategy**:
    - Use `debounce` for highly dynamic apps (SPAs with frequent updates)
    - Use `fixed` for predictable apps with controlled updates
